...
Future<void> main() async {
  ...
  try {
    ...
    final sum = await render.workflows.runTask('$slug/sumSquares', [
      [2, 3, 4],
    ...
    check(
      'Dart message survived',
      boom.error?.contains('sku-42') ?? false,
      boom.error?.split('\n').first,
    );
    ...
    try {
      ...
      check(
        'unknown run id raises with a usable hint',
        e.hint?.contains('trn-') ?? false,
        '${e.statusCode}: ${e.hint}',
      );
    }
